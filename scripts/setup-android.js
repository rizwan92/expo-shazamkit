...
function setupAndroid() {
  try {
    ...
    if (!fs.existsSync(androidAppDir)) {
      console.error(
        "❌ Error: This doesn't appear to be a React Native project.",
      );
      console.error(
        "   Make sure you're running this script from your project root.",
      );
      process.exit(1);
    }

    // Find the module's AAR file
    const moduleAAR = path.join(
      projectRoot,
      "node_modules",
...
function updateBuildGradle(projectRoot) {
  try {
    ...
    if (match) {
      ...
      console.log(
        "   Please manually add this line to your dependencies block:",
      );
      console.log(
        "   implementation files('libs/shazamkit-android-release.aar')",
      );
    ...
    console.log(
      "⚠️  Please manually add this line to your dependencies block:",
    );
    console.log(
      "   implementation files('libs/shazamkit-android-release.aar')",
    );
...
if (args.length > 0 && args[0] === "--help") {
  ...
  console.log(
    "1. Copy the ShazamKit Android AAR file to your app's libs directory",
  );
